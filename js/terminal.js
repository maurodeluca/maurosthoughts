...
let awareness = parseFloat(localStorage.getItem('awareness'));
let history = JSON.parse(localStorage.getItem('history'));
let state = localStorage.getItem('state');
let booted = localStorage.getItem('booted') === 'true';
let privileged = localStorage.getItem('privileged') === 'true';
let isAscended = localStorage.getItem('isAscended') === 'true';
let isTranscended = localStorage.getItem('isTranscended') === 'true';
let minimalMode = localStorage.getItem('minimalMode') === 'true';
let sudoMode = localStorage.getItem('sudoMode') === 'true';
let godMode = localStorage.getItem('godMode') === 'true';
let unstableMode = localStorage.getItem('unstableMode') === 'true';
let unstableGlitches = [];
let awarenessGlitchInterval = null;
let historyIndex = 0;
const sudoColor = '#00FFAA';
const redColor = '#b11212';
const goldColor = '#FFAA00';
...
document.addEventListener('DOMContentLoaded', async () => {
    localStorage.setItem('booted', booted || 'false');
    localStorage.setItem('awareness', awareness || 0);
    localStorage.setItem('sessionStart', sessionStart || Date.now());
    localStorage.setItem('state', state || 'normal');
    localStorage.setItem('privileged', privileged || 'false');
    localStorage.setItem('isAscended', isAscended || 'false');
    localStorage.setItem('isTranscended', isTranscended || 'false');
    localStorage.setItem('minimalMode', minimalMode || 'false');
    localStorage.setItem('sudoMode', sudoMode || 'false');
    localStorage.setItem('godMode', godMode || 'false');
    localStorage.setItem('unstableMode', unstableMode || 'false');
    if (!history) history = [];
...
helpBtn.addEventListener('click', async (e) => {
    ...
    sessionStart = Number(localStorage.getItem('sessionStart'));
    awareness = parseFloat(localStorage.getItem('awareness'));
    history = JSON.parse(localStorage.getItem('history'));
    state = localStorage.getItem('state');
    booted = localStorage.getItem('booted') === 'true';
    privileged = localStorage.getItem('privileged') === 'true';
    isAscended = localStorage.getItem('isAscended') === 'true';
    isTranscended = localStorage.getItem('isTranscended') === 'true';
    minimalMode = localStorage.getItem('minimalMode') === 'true';
    sudoMode = localStorage.getItem('sudoMode') === 'true';
    godMode = localStorage.getItem('godMode') === 'true';
    unstableMode = localStorage.getItem('unstableMode') === 'true';
...
function showAwareness(message, color) {
    let awarenessDiv = overlay.querySelector('#awareness');

    // If it doesn't exist, create it
    if (!awarenessDiv) {
        awarenessDiv = document.createElement('div');
        awarenessDiv.id = 'awareness';
    ...
    awarenessDiv.classList.remove('pulse');
...
async function openTerminal() {
    if (!overlay.classList.contains('active')) {
        ...
        if (!booted) {
            ...
            printStateMessage();
            setTimeout(async() => await print("reawakening..."), 200);
        }
    }
}
...
async function boot() {
    ...
    if (localStorage.getItem('godMode') === 'true') {
        localStorage.setItem('godMode', false);
        godMode = false; // temporarily allow toggling god mode on page load;
        toggleGodMode();
    }
    ...
    printStateMessage();
    setTimeout(async() => await print("booting consciousness..."), 200);
    setTimeout(async() => await print('type "help"'), 1000);
    booted = true;
...
function toggleSudoMode() {
    ...
    sudoMode = !sudoMode;
    localStorage.setItem('sudoMode', sudoMode);
...
function toggleUnstableMode() {
    ...
    localStorage.setItem('unstableMode', unstableMode);
...
function unstableLoop(timestamp) {
    ...
    const textElements = document.querySelectorAll('h1, h2, h3, h4, h5, h6, p, nav');
